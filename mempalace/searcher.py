...
def _open_search_collection(palace_path: str, collection_name: str):
    try:
        ...
        # Fork fix #9: don't mislabel a genuinely-unexpected failure as "No
        # palace found". The specific recoverable conditions above (backend
        # mismatch, unknown backend, missing palace, backend error) each
        # return their own diagnostic dict; anything that reaches here is
        # unexpected. Log the full chained traceback so the real failure mode
        # lands in the logs, then propagate with the original exception type
        # so callers can distinguish the failure mode programmatically.
        logger.exception(
